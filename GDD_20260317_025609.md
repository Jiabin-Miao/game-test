...
#### 4.3.1 3位Boss（示例：`{{BOSS_1_NAME}}`、`{{BOSS_2_NAME}}`...）
1. **`{{BOSS_1_NAME}}`**
    - **特点**：`{{BOSS_TRAIT_1}}`
    - **战斗风格**：`阶段制`
    - **核心机制**：`阶段转换`
...
- **列表样式**: `分割线式`
- **弹窗样式**: `半透明蒙层`
- **字体规范**: `黑体/16px-32px/1.5倍行距`（字体家族/字号层级/行距）
- **间距规范**: `8px基础单位`（统一间距单位）
...
#### 5.5.3 图形规范
- **图标风格统一性**: `统一线条粗细与圆角`
- **色彩编码规则**: `红=危险、绿=安全、黄=警告`（如：红色=警告/绿色=成功）
